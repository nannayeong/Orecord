...
function changingTableFunc(obj) {
  ...
  for (var i=0; i<resultLog.length; i++) {
	...
	var html = '<div class="table" id="changingTableId">';
			...
			html += '		<tr><th width="30%">날짜</th><th width="40%">내가 후원한 유저</th><th width="30%">후원포인트</th></tr>';
			html += '	</thead>';
			html += '	<tbody>';
	if (obj.totalRecord == 0) {
		...
		for (var i=0; i<resultLog.length; i++) {
		    html += '<tr><td>' + resultLog[i].regidate + '</td><td>' + resultLog[i].patronId
