			...
			<table style="width:100%;border:2px #f2f2f2 solid;margin:auto;margin-bottom:1em" class="feed">
						<tr>
							<td rowspan="5" style="width:7em;padding-left:1em;padding-right:1em;padding-top:1.5em;vertical-align:top">
								<img src="${b.value.imagename }" alt="" style="width:6em"/>
							</td>
							<td style="padding-left:1em">
								...
								<div style="font-size:18px">
									...
									- <a href="${pageContext.request.contextPath}/search.do?searchWord=${b.value.artistname}">${b.value.artistname}</a>
								</div>
							</td>
							<td style="text-align:right; color:#423e3e; font-size:14px; padding-right:2em">
								<div>${b.value.regidate }</div>
								<div>${b.value.category }</div>
							</td>	
							<td colspan="2" style="padding-right:0.5em">
								<div>
 									<div class="dropdown">
									  ...
									  <c:if test="${pageContext.request.userPrincipal.name eq b.value.id }">
									    <a class="dropdown-item" href="javascript:recordDeleteFunc(${b.value.audio_idx });">삭제하기</a>
									    <a class="dropdown-item" href="#">수정하기</a>
									    <c:if test="${b.value.party eq 1}">
						...
						<tr>
							<td colspan="3">
								<audio controls style="width:97%" id="${b.value.albumName }">
									<source src="${b.value.audiofilename }" type="audio/mp4">
								</audio>
							</td>
						</tr>
						<tr>
        		...
        		<c:set var="likeB" value="true"/>
					...
					<c:choose>
               			<c:when test="${likeB}">
                      		<button type="button" class="btn btn-outline-secondary btn-sm heart ${b.value.audio_idx}" title="좋아요" onclick="heartbtn('${b.value.audio_idx}')" name="minibtn">
                      		좋아요 &nbsp
                      		${b.value.like_count }
                      		</button> 
                 		</c:when>
                 <c:otherwise>
                      <button type="button" class="btn btn-secondary btn-sm heart ${b.value.audio_idx}" title="좋아요" onclick="heartbtn('${b.value.audio_idx}')"  name="minibtn">
                      좋아요 &nbsp
                      ${b.value.like_count }
                      </button> 
                 </c:otherwise>
               		</c:choose>
						<button type="button" class="btn btn-secondary btn-sm" id="addplaylist" onclick="logincheck(this);" data-toggle="modal" data-target="#play${b.value.audio_idx}">
							플레이리스트
						</button>
						<!-- The Modal -->
						<c:if test="${not empty pageContext.request.userPrincipal.name}">
						<div class="modal" id="play${b.value.audio_idx}">
						  <div class="modal-dialog">
						    <div class="modal-content">
						      ...
						      <div class="modal-body" style="text-align:center">
						      	<div><img src="${b.value.imagename }" alt="" style="width:25px"/> 
									${b.value.audiotitle } - ${b.value.artistname }
								</div><br />
						...
						<c:if test="${pageContext.request.userPrincipal.name eq b.value.id and b.value.party eq 1}">
						<button type="button" class="btn btn-secondary btn-sm" onclick="coOp('${b.value.audio_idx}')">참여</button>
						</c:if>
							</td>
							<td style="text-align:right;padding-right:1.5em;color:#423e3e;font-size:14px" colspan="2">
								재생 : ${b.value.play_count} &nbsp&nbsp
								댓글수 : ${b.value.commentCount }
							</td>
						</tr>
						<tr>
							<td colspan="3">
