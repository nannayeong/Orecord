...
<body class="">
  ...
  <div class="wrapper">
  	<!-- 사이드바 -->
    <%@include file="/resources/include/left_searchbar.jsp" %>
           ...
           <div class="card-header">
             <div class="row">
               ...
               <div class="col-lg-7 text-left">
                 <h5 class="card-category">${ b.value.category} &nbsp;&nbsp; <a href="">${ b.value.artistname}</a> </h5>
                 <h3 class="card-title"><a href="./board/view.do">${ b.value.audiotitle}</a></h3>
               </div>
               <!-- 본문 제목 종료 -->
               <!-- 본문 내부 버튼 -->
               ...
               <c:set var="followB" value="false"/>
        		<c:forEach var="f" items="${follows1 }">
        		...
        		<c:if test="${b.value.id ne id}">
               <c:choose>
               <c:when test="${followB}">
                 <button type="button" class="minibtn following ${b.value.id}" id="follow${b.value.id}" title="팔로우" onclick="fBtn('${b.value.id}')" 
                 	name="${b.value.id}"><i class="tim-icons icon-link-72"></i></button>
                 </c:when>
                 <c:otherwise>
                 <button type="button" class="minibtn unfollowing ${b.value.id}" id="follow${b.value.id}" title="팔로우" onclick="fBtn('${b.value.id}')" 
                 	name="${b.value.id}"><i class="tim-icons icon-link-72"></i></button>
                 </c:otherwise>
               </c:choose>
                </c:if>
                
                 <button type="button" class="minibtn" title="후원" onclick="donatebtn('${b.value.id}')" name="donate-btn"><i class="tim-icons icon-gift-2"></i></button>
               <c:choose>
               <c:when test="${likeB}">
                      <button type="button" class="minibtn heart ${b.value.audio_idx}" title="좋아요" onclick="heartbtn('${b.value.audio_idx}')" name="heart-btn"><i class="tim-icons icon-heart-2"></i></button> 
                 </c:when>
                 <c:otherwise>
                      <button type="button" class="minibtn noheart ${b.value.audio_idx}" title="좋아요" onclick="heartbtn('${b.value.audio_idx}')" name="heart-btn"><i class="tim-icons icon-heart-2"></i></button> 
                 </c:otherwise>
               </c:choose>
                 <button type="button" class="minibtn" title="플레이리스트 추가" onclick="addlistbtn('${b.value.audio_idx}')" name="addlist-btn"><i class="tim-icons icon-bullet-list-67"></i></button>
               </div>
               <div class="miniinfos">
             ...
             <div class="card-body">
           	<h4> <a href="">댓글 
         	<c:forEach var="d" items="${commentC1}">
         ...
         <div class="card">
           <div class="card-header">
             <div class="row">
               ...
               <!-- 하나라도 일치하는 팔로우 정보가 있으면 followB가 true로 바뀜 -->
               <c:set var="followB" value="false"/>
        		<c:forEach var="f" items="${follows2 }">
        		<c:if test="${pageContext.request.userPrincipal.name eq f.user_id and b.value.id eq f.following_id}">
        		<c:set var="followB" value="true"/>
        		</c:if>
        		</c:forEach>
             ...
             <div class="card-body">
           	<h4> <a href="">댓글 
         	<c:forEach var="d" items="${commentC2}">
           	<c:if test="${b.value.audio_idx eq d.key}">
           	${d.value }
           	</c:if>
           	</c:forEach>
         ...
         <div class="card">
           <div class="card-header artist">
               ...
               <div class="artist photo">
                 <img class="sidebarimg" src="./resources/assets/img/anime3.png" alt="Profile Photo">
               </div>
                <div class="artist profile">
                <h3 class="nickname">${a.nickname}</h3>
                <h4> 게시글 XX개 </h4>
               </div>
               <!-- 본문 제목 종료 -->
               <!-- 본문 내부 버튼 -->
               <div class="artist subinfo">
               <div class="minibuttons">
               <!-- 하나라도 일치하는 팔로우 정보가 있으면 followB가 true로 바뀜 -->
               <c:set var="followB" value="false"/>
               ...
               <c:choose>
               <c:when test="${followB}">
                 <button type="button" class="minibtn following ${a.id}" id="follow${a.id}" title="팔로우" onclick="fBtn('${a.id}')" 
                 	name="${a.id}"><i class="tim-icons icon-link-72"></i></button>
                 </c:when>
                 <c:otherwise>
                 <button type="button" class="minibtn unfollowing ${a.id}" id="follow${a.id}" title="팔로우" onclick="fBtn('${a.id}')" 
                 	name="${a.id}"><i class="tim-icons icon-link-72"></i></button>
                 </c:otherwise>
               </c:choose>
               </c:if>
         ...
         <div class="card">
           <div class="card-header">
             <div class="row">
               ...
               <!-- 하나라도 일치하는 팔로우 정보가 있으면 followB가 true로 바뀜 -->
               <c:set var="followB" value="false"/>
        		<c:forEach var="f" items="${follows3 }">
        		<c:if test="${pageContext.request.userPrincipal.name eq f.user_id and b.value.id eq f.following_id}">
        		<c:set var="followB" value="true"/>
        		</c:if>
        		</c:forEach>
             ...
             <h4>${b.value.contents}</h4>
           </div>
             <div class="card-body">
           	<h4> <a href="">댓글  
         	<c:forEach var="d" items="${commentC3}">
           	<c:if test="${b.value.audio_idx eq d.key}">
           	${d.value }
           	</c:if>
           	</c:forEach>
